...
/// The default config location: `<config_dir>/qiosq/config.toml` (e.g.
/// `~/.config/qiosq/config.toml` on Linux). `None` if the OS config dir is
/// unknown. `qtui` loads this when no `--config` is given, and `qtui init`
/// writes here.
pub fn default_config_path() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("qiosq").join("config.toml"))
}

/// Errors from [`write_config`].
#[derive(Debug, Error)]
pub enum WriteError {
...
/// Write a minimal, valid qtui config to `path` with the given Quiqr data
/// directory and sensible defaults (mirroring the shipped example). Creates
/// parent directories. Refuses to overwrite an existing file unless `force`.
/// The result is guaranteed to load + validate.
pub fn write_config(path: &Path, quiqr_data_dir: &str, force: bool) -> Result<(), WriteError> {
    ...
    std::fs::write(path, render_config(quiqr_data_dir)).map_err(|source| WriteError::Io {
        path: path.to_path_buf(),
        source,
    })
}

/// Render a minimal valid config TOML for `quiqr_data_dir` with defaults that
/// match validation (port range excludes 13131; non-empty agent fields).
fn render_config(quiqr_data_dir: &str) -> String {
    // Escape backslashes + quotes for the TOML basic string.
    let dir = quiqr_data_dir.replace('\\', "\\\\").replace('"', "\\\"");
    format!(
        r#"# Generated by `qtui init`. Edit freely; see config/quiqr-tui.example.toml
# for the full set of options and comments.
...
"#;
    ...
    #[test]
    fn default_config_path_ends_with_qiosq_config() {
        // May be None in a bare sandbox without HOME/XDG; only assert the shape
        // when present.
        if let Some(p) = default_config_path() {
            assert!(p.ends_with("qiosq/config.toml"), "got {p:?}");
        }
    }

    #[test]
    fn write_config_roundtrips_and_does_not_clobber() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("nested").join("config.toml");
        let dir = tmp.path().join("data");

        // Writes (creating parent dirs) a config the loader accepts.
        write_config(&path, &dir.to_string_lossy(), false).unwrap();
        let cfg = Config::load_and_validate(&path).expect("written config validates");
        assert_eq!(cfg.storage.quiqr_data_dir, dir.to_string_lossy());

        // Refuses to overwrite without force.
        let err = write_config(&path, "/other", false).unwrap_err();
        assert!(matches!(err, WriteError::Exists(_)));

        // force=true overwrites.
        write_config(&path, "/other/data", true).unwrap();
        let cfg = Config::load_and_validate(&path).unwrap();
        assert_eq!(cfg.storage.quiqr_data_dir, "/other/data");
    }
}
